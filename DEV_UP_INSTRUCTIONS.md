...
**Repository:** `GlacierEQ/elastic-trajectory-index-layout`  
**Company lens (independent):** Elastic (`elastic`)  
**Innovation:** Trajectory Index Layout  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Elastic affiliation, proprietary access, or production deployment.
...
Cluster spans/tool calls by trajectory id + time so forensic queries avoid full-mesh joins.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Elastic employment, endorsement, or proprietary systems access.
...
File: `src/trajectory_index_layout.py`

- Expand `TrajectoryIndexLayout` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_trajectory_index_layout.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_trajectory_index_layout.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Elastic APIs
